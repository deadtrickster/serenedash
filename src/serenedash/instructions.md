...
**Confirming you have this document at all.** A client can connect to the server, list its tools and call them successfully while never injecting this text - the tools work either way, so nothing about the answers looks wrong. If the user asks whether your serenedash instructions loaded, do not say yes because the tools respond. Answer with two things that exist ONLY here and in no tool result: the revision `{{REVISION}}` above, and the name of the rule in "The denominator rule" below. If you cannot produce them from context, say so plainly - the client did not inject them, and reading `serenedash://instructions` is the fix.
...
- **`parse` means the statements are expensive to read**, not that the data is expensive to process. It is the PEG grammar's matchers, and it is per-statement rather than per-row. The symbols are `duckdb::ListMatcher::MatchParseResult`, `PEGParser`, `PEGTransformer`, `MatcherFactory` and friends: **`Matcher` in a symbol name here is a grammar matcher, not a search match and not a join match.** `RowMatcher` (hash join) and `ExpressionMatcher` (optimizer) are different things again. Reading `ListMatcher::MatchParseResult` as "the BM25 query is expensive" is the mistake this bullet exists to stop - the engine label in the tool output is the answer, not the symbol name. The usual cause is a large literal: a 1024-dimensional embedding sent as SQL text is about 21,700 characters, re-parsed on every execution. This deployment does exactly that — 31% of a 68 KB hybrid-search statement is one float literal — and the fix is client-side, binary bind parameters instead of interpolated text. Check it with `SELECT length(query) FROM pg_stat_activity` and look at what the statement actually carries.
...
- **That last sentence is a compression argument and nothing else.** It applies when checkpoints are landing and you are paying for them too often. If the WAL is far past the threshold, checkpoints are *not* landing, and raising the threshold licenses the WAL to grow further while fixing nothing. Read the WAL size before you read this paragraph as advice. A model reading this document did exactly that on a WAL 1760x over threshold and recommended raising it.
...
- **Temp files are deleted in a destructor and never swept at startup.** A clean shutdown runs that destructor and removes what the exiting process held, so an orderly restart leaks nothing. A killed one - OOM, `kill -9`, a container stop that times out - leaks what it was holding, forever. And because nothing sweeps the directory at startup, files from an earlier unclean exit survive every later restart however clean it was. So "restart to clear the temp directory" is wrong twice over: it does not sweep, and if the restart is not clean it adds to the pile.
